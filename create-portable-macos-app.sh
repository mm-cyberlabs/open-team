...
# Script to create a fully portable macOS .app bundle for OpenTeam
# This creates a self-contained app with embedded JRE - no Java installation required on target
...
echo "🔨 Creating self-contained macOS app with jpackage..."

# Use jpackage to create the app bundle with embedded JRE
jpackage \
...
if [ -d "$APP_DIR" ]; then
    ...
    echo "3. Create ZIP for distribution: cd target && zip -r ${APP_NAME}-${APP_VERSION}-macOS-portable.zip ${APP_NAME}.app"
    echo ""
